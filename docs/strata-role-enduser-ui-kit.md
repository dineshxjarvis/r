...
## Global elements shared by every role (not repeated per end-user)
...
| [Close Visit] | Locks the visit for edits | `POST /inspection-visits/{id}/actions {action:"CLOSE"}` | Visit card marked COMPLETED |
| [Prepare Report] | Compiles findings + observations into a report draft | `POST /inspections/{id}/actions {action:"PREPARE_REPORT"}` | Report preview screen |
...
| description typing (debounced) | Requests AI suggestion | `POST /analytics/actions {action:"SUGGEST_CLAUSE", payload:{description}}` |
...
| [Acknowledge] | Acks a regulatory finding in place | `POST /findings/{id}/actions {action:"ACKNOWLEDGE"}` | row badge updates |
...
| Element | Click behavior | API |
|---|---|---|
| [Update CAPA Progress] | Opens progress-note form | `POST /capas/{id}/actions {action:"LOG_PROGRESS"}` |
| [Submit CAPA Completion] | Opens evidence submission for the CAPA | `POST /capas/{id}/actions {action:"SUBMIT"}` + `POST /evidence/sync` |
| [Acknowledge] | Records acknowledgement (not an admission — copy says so) | `POST /findings/{id}/actions {action:"ACKNOWLEDGE"}` |
...
| Geo-boundary check (automatic) | Validates photo location vs lease | `GET /geospatial/evaluate?subject=evidence&target=lease_boundary` |
...
### Screen 1 — Dashboard
**Route:** `/field/safety/dashboard` · **Endpoint:** `GET /dashboard?view=personal_queue&domain=safety` + `GET /analytics/signals?filter[mine_id]=…&filter[domain]=safety`
...
| [View attendance records] | Opens ventilation pre-shift log | `GET /attendance/exceptions?filter[mine_id]=…` | Attendance exceptions view |
...
| Element | Click behavior | API | Lands on |
|---|---|---|---|
| [View] (consent tile) | Opens CTO/CTE record | `GET /regulatory-instrument-versions?filter[subject_mine_id]=…&view=current` | Consent detail |
...
| Monitoring value | Opens time-series chart | `GET /environment/monitoring-data?filter[mine_id]=…&parameter=…` | Monitoring drill-down |
...
**Route:** `/field/attendance` · **Endpoint:** `GET /attendance/shifts?filter[mine_id]=…&filter[date]=today`
...
| Element | Click behavior | API |
|---|---|---|
| [View absentees] | Opens absentee list | `GET /attendance/shifts/{id}?expand=absentees` |
| [View roster] | Opens contractor roster | `GET /contractor-roster-versions?filter[mine_id]=…` |
| [Reconcile muster] | Opens exception-resolution form | `GET /attendance/exceptions?filter[mine_id]=…&filter[unresolved]=true` |
| [Review →] (roster) | Opens roster for approval | `GET /contractor-roster-versions/{id}` |
| [Attest daily attendance] | Signs off the day's register | `POST /attendance/shifts/{id}/actions {action:"ATTEST"}` |
...
**Route:** `/field/grievances` · **Endpoint:** `GET /grievances?filter[mine_id]=…`
List of raised grievances with status; [+ New Grievance] → `POST /grievances`; row click → `GET /grievances/{id}` detail with resolution timeline.
...
| [Acknowledge] | Acks the finding | `POST /findings/{id}/actions {action:"ACKNOWLEDGE"}` | card updates in place |
...
**Interactions:** domain tab → re-queries `group_by`; table row → `GET /obligation-instances/{id}` detail; [Export PDF] → `GET /reports/exports?type=compliance_register&mine_id=…`.
...
| Element | Click behavior | API |
|---|---|---|
| [Approve] | Verifies the CAPA closed | `POST /capas/{id}/actions {action:"VERIFY", verdict:"SATISFIED"}` |
| [Reject with reason] | Sends CAPA back with a mandatory note | `POST /capas/{id}/actions {action:"REJECT", reason:"…"}` |
...
**Interactions:** [Review roster →] opens `GET /contractor-roster-versions?filter[status]=PENDING_REVIEW&filter[mine_id]=…`; [Approve] on that roster fires `POST /contractor-roster-versions/{id}/actions {action:"APPROVE"}`.
...
**Interactions:** [Generate Report] → opens report-type/period picker → `POST /reports {type, mine_id, period}`; once ready, [Download] → `GET /reports/{id}/download`; [Upload Doc] → `POST /documents`.
...
**Route:** `/mine/grievances` · **Endpoint:** `GET /grievances?filter[mine_id]=…`
...
**Interactions:** [Assign] → `POST /grievances/{id}/actions {action:"ASSIGN"}`; [Escalate] → `POST /grievances/{id}/actions {action:"ESCALATE"}`; [Close] (in detail view) → `POST /grievances/{id}/actions {action:"CLOSE"}`.
...
**Endpoint:** `GET /dashboard?view=measures&scope_type=MINE&scope_id={mine_id}&domain=PRODUCTION` plus `GET /production?filter[mine_id]=…&period=…` for the returns strip. Interactions follow the same drilldown pattern as §2.1 Screen 1.

### Screen 2 — Production Returns & Filings
**Route:** `/mine/production` · **Endpoint:** `GET /production?filter[mine_id]=…&period=…`
...
**Interactions:** [Submit Return] → `POST /production/actions {action:"SUBMIT_RETURN"}`; discrepancy row → `GET /production/{id}?expand=reconciliation`.
...
**Route:** `/corporate/analytics` · **Endpoint:** `GET /analytics/signals?filter[scope]=organization_unit&filter[scope_id]={id}`
...
| [Review & approve] (per clause) | Human confirms each proposed clause | `POST /extractions/{id}/actions {action:"APPROVE"}` |
...
**Interactions:** cell click → `GET /obligation-instances?filter[mine_id]=…&filter[domain]=…` (obligations table for that mine/domain); [Export] → `GET /reports/exports?type=compliance_portfolio&org_unit_id=…`.
...
**Route:** `/corporate/reports` · **Endpoint:** `GET /reports?filter[org_unit_id]=…`
...
**Interactions:** [Generate New] → `POST /reports {type, org_unit_id, period}`; [Schedule ↻] → `POST /reports/schedules` (recurring); [Prepare]/[Download]/[View] → `GET /reports/{id}` / `GET /reports/{id}/download`.
...
| [🗺 View mine map…] | Opens jurisdiction map | `GET /geospatial/layers?filter[jurisdiction_id]=…` | Jurisdiction Map screen |
...
**Interactions:** [Issue decision] → `POST /regulatory-cases/{id}/actions {action:"DECIDE"}`; [Lift order] → `POST /regulatory-cases/{id}/actions {action:"LIFT"}`; [Request clarification] → `POST /regulatory-cases/{id}/actions {action:"REQUEST_INFO"}`.
...
**Route:** `/regulatory/map` · **Endpoint:** `GET /geospatial/layers?filter[jurisdiction_id]=…`
...
**Route:** `/regulatory/instruments` · **Endpoint:** `GET /regulatory-instrument-versions?filter[authority_id]=…`
...
**Interactions:** [+ New Order] → `POST /regulatory-instrument-versions`; row → `GET /regulatory-instrument-versions/{id}`, which shows every mine/obligation the instrument has propagated to (this is how a new circular from this authority becomes obligations at each mine, mirroring the Document Intelligence flow in §3.1 Screen 4 but issued by the regulator rather than uploaded by the operator).
...
**Interactions:** [Submit →] opens roster upload → `POST /contractor-roster-versions`; [Upload] on compliance alert → `POST /documents` then linked to the requirement instance → `POST /contractor-requirement-instances/{id}/actions {action:"SUBMIT"}`.
...
### Screen 5 — Grievances
**Route:** `/contractor/grievances` · **Endpoint:** `GET /grievances?filter[organization_id]=…`

List of grievances raised by or on behalf of this contractor's workforce, same card pattern as §2.1 Screen 10. **[+ New Grievance]** → `POST /grievances` (filed on behalf of a worker).
...
**Route:** `/contractor/workers` · **Endpoint:** `GET /package-assignments?filter[package_id]=…`
...
**Interactions:** [Update document] → `POST /contractor-requirement-instances/{id}/actions {action:"SUBMIT", payload:{document_id}}`; [Remind] → `POST /notifications`; [Prepare next roster] → `POST /contractor-roster-versions`.
...
### Screen 5 — Grievances
**Route:** `/contractor/grievances` · **Endpoint:** `GET /grievances?filter[package_id]=…`
...
- **GIS Mine Map (`/{role}/map`)** — `GET /geospatial/layers` + `GET /geospatial/features`. Layer toggles: mine boundary, bench layout, haul roads, active findings, inspections, attendance zones, environmental monitoring, contractor zones. Clicking a map pin opens that record's detail screen.
...
- **Grievance Intake (`/{role}/grievances`)** — available to Field-level, and Contractors. `GET /grievances` / `POST /grievances`.
...
*Document version 2.0 — restructured Role → End-User → Complete UI Kit format. All endpoint references correspond to the API specification (`docs/api-specs/endpoints/`) in the Strata backend repository.*
